...
1. **Completed at `df6e68c0`:** make the validation signal trustworthy before further product-side stereo work. The validators reject reverse-Z, missing IMM content, duplicated stereo views, sky-only eyes, incorrect cyan occlusion, black/default scenes, and graphics-API fallback. Every validation step includes an authoritative visual check; logs remain fast-fail diagnostics only.
2. **Completed with run `30892709675`:** run the full suite at an exact revision, manually inspect every capture, and reconcile every automatic result with the image. The Android external-display and macOS Editor Play false failures are removed and cloud-proven.
3. **Current:** fix the two genuine Unity Android Vulkan failures exposed by the trustworthy suite. Repair Quest Multi Pass so both eyes contain distinct, correct IMM content, make the Firebase synthetic-stereo lane exercise the same production two-eye producer/presentation path, and repair the cyan full-depth occlusion defect without changing the working Metal, OpenGL, DirectX, Windows Vulkan, or non-XR paths.
4. Fix the remaining genuine rendering/composition defects: Godot Vulkan full-depth/sky-only output, Godot ordered-overlay occlusion, Unity Metal ordered-overlay probe loss, and the cross-renderer cyan depth leakage. IMM uses discard/stencil, MSAA coverage, and stippled OIT; ordinary `SRC_ALPHA / ONE_MINUS_SRC_ALPHA` compositing is not a valid substitute.
...
- Run `30897579019` includes that rebuilt patch. Its non-XR synthetic presented eyes remain byte-identical, so the patch has not yet proven the Unity presentation handoff. The next run captures both native eye RenderTextures as a separate side-by-side artifact before presentation; native-identical versus presented-identical results will distinguish matrix/render failure from Unity composition failure.
...
Current implementation status: the validator contracts for the reviewed Android Godot depth defects, missing Godot IMM content, duplicated stereo views, sky-only eyes, and cyan leakage are present. Unity Metal and Godot ordered-overlay rendering are still product failures, not validator false positives. The temporary generic alpha-blend compositor change was reverted because IMM's OIT/coverage model does not expose ordinary straight-alpha scene compositing.
...
3. Replaying the corrected contracts against the exact cloud artifacts reports the DirectX render as successful and its cyan leakage as `composition_failed`; macOS Unity reports its full-depth cyan leakage and missing ordered-overlay probes as `composition_failed`; macOS Godot, Windows Godot, and Android Unity retain genuine visual failures. The Windows synthetic-stereo lane remains correctly `runtime_failed` when hosted Unity rejects Vulkan and falls back to Direct3D.
...
8. Run `30892709675` cloud-proves the Android external-video correction. The external-display validator passes two consecutive physical-screen samples at reported times 20 and 22 seconds, and the selected `unity-android-vulkan-external-render.png` visibly contains the correct IMM scene without composition probes. The independent render-only capture is also correct. Android remains red only for two genuine defects: cyan depth leakage in the composition capture and byte-identical synthetic eye images.
   Run `30897579019` exposed a remaining observation-cadence edge case: the 20-second sample missed the correlation floor by `0.001`, the 22-second sample passed, and the fixture transitioned to composition probes by 24 seconds. The visual contract and two-consecutive-sample rule remain unchanged; video sampling is now once per second so the stable render interval receives multiple observations instead of weakening the validator.
9. The first macOS job attempt in run `30892709675` ended before rendering when Unity's license-activation process aborted during shutdown with exit code 134. A targeted rerun reached the readiness gate at frame 229, wrote `unity-macos-metal-editor-play.png`, and passed the macOS Metal render contract. A second targeted rerun was downloaded before aggregate cleanup and manually inspected: the Editor Play image contains the expected IMM character and branch scene and closely matches the separately captured Metal player render. The macOS lane remains red only for visible product defects: cyan leakage across the character in full-depth mode and absent magenta/yellow probes in ordered-overlay mode.

False-visual-failure cleanup is complete for the normal push workflow at `df6e68c0`. All 17 supported matrix rows have evidence or an explicit runtime/coverage state; every produced image in the exact-revision run has been reconciled with its automatic verdict. Remaining red visual lanes are actionable rendering, composition, or stereo defects. Hosted Windows Unity Vulkan remains an explicit API/runtime coverage failure because Unity rejects Lavapipe and falls back to Direct3D, and the hardware-gated Windows Unity Vulkan composition jobs remain explicit incomplete coverage rather than false visual results.

Run `30867812844` confirmed the DirectX false negative is removed and the stricter full-depth contract catches the previously false-passing macOS Godot image. Its aggregate artifact was manually audited image by image: all green visual lanes contain recognizable IMM content; Windows Godot full-depth is sky-only; Windows Godot ordered overlay has cyan in front; Unity DirectX, Unity Metal, and Unity Android contain genuine cyan leakage; Android synthetic stereo contains two valid but byte-identical eyes; and hosted Windows Unity Vulkan is an API-fallback runtime failure. Follow-up run `30869412758` cloud-proved that the narrowed ordered-overlay region excludes legitimate cyan butterfly content while continuing to reject the actual Windows Godot cyan square. Run `30892709675` then cloud-proved both the Android physical-screen correction and the readiness-gated macOS Editor Play capture. The correction passes all 31 local CI-tool self-tests, the 45-job workflow-matrix check, and the sample-entrypoint contract.
