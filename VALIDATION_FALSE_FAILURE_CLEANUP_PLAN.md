...
2. The generic `sample1-composition-content` contract checked only broad IMM content. It therefore allowed a macOS Godot composition image with no magenta/yellow probes to pass. All full-depth lanes now use a dedicated `sample1-full-depth` contract requiring recognizable IMM content, both visible probes, and near-zero cyan leakage in the reviewed occlusion region. The ordered-overlay cyan region is also wide enough to reject the subtle leakage visible in the current Unity captures.
...
These changes pass local contract, classifier, workflow-matrix, and YAML tests. They are awaiting one final exact-revision cloud run and a second manual image/verdict reconciliation before the cleanup gate can close.
