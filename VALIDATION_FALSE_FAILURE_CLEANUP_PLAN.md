...
Full run `30865518088`, attempt 2, at `8530bc98` is the latest manually audited exact-revision run. Its individual job artifacts were inspected as each job completed. The audit found two remaining signal defects and several genuine product failures:
...
6. The combined licensed macOS Unity Editor invocation reaches Play mode, but `CaptureScreenshotAsTexture` returns no pixels when called from `EditorApplication.update` before the end-of-frame capture point. The capture helper now renders the active camera into a temporary 1280x720 `RenderTexture` and performs synchronous `ReadPixels`, restoring all camera/render-target state afterward. This exact fix still requires cloud confirmation.

Run `30867812844` confirmed the DirectX false negative is removed and the stricter full-depth contract catches the previously false-passing macOS Godot image. Its aggregate artifact was manually audited image by image: all green visual lanes contain recognizable IMM content; Windows Godot full-depth is sky-only; Windows Godot ordered overlay has cyan in front; Unity DirectX, Unity Metal, and Unity Android contain genuine cyan leakage; Android synthetic stereo contains two valid but byte-identical eyes; and hosted Windows Unity Vulkan is an API-fallback runtime failure. The follow-up ordered-overlay region and Editor Play capture changes pass local contract, classifier, workflow-matrix, and YAML tests but require one final exact-revision cloud confirmation before the cleanup gate can close.
