...
2. The attempted GameCI `manualExit` solution did not produce the macOS Unity Editor Play capture. The player build and Editor Play smoke are now separate Unity invocations: GameCI builds and exits normally, then the installed Unity 6.3 LTS editor is launched directly without `-quit`; the existing asynchronous callback captures the Game view and calls `EditorApplication.Exit`. A lane-level classifier requires the Editor Play, render-only, full-depth, and ordered-overlay images and metrics. Preliminary player, metric, report, and log steps cannot independently override that final visual verdict.
