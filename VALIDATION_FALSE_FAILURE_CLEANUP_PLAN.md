...
Full run `30858852665` was manually audited. It confirmed that the standalone render lanes and browser player are visually correct, while the remaining Unity/Godot depth, missing-content, Vulkan-fallback, and duplicated-stereo failures are real. It also exposed three false-signal paths that are now fixed locally but still require cloud confirmation:

1. Nested `captures/editor-play` and `captures/sample-play` directories were rediscovered as independent lanes, then inherited an unrelated sibling composition failure. A lane-local render report now owns all of its nested capture modes, and regression fixtures forbid standalone `Editorplay` and `Sampleplay` sections.
2. Android Godot produced both required device images and both external visual metrics passed, but a missing redundant `IMM Godot Vulkan visual smoke passed` marker made the instrumentation wrapper return `-1`; the manifest then hard-coded this as `runtime_failed`. A dedicated Android Godot classifier now makes the two visual contracts authoritative, preserves crash/device-loss evidence as runtime failures, reports missing captures as evidence/infrastructure failures, and records complete-run Firebase/log errors as supporting warnings. The preliminary Firebase and metric steps are `continue-on-error`; the classifier is the final gate.
3. The Unity macOS Editor Play smoke scheduled an asynchronous capture inside GameCI while GameCI supplied `-quit`, so Unity exited immediately after entering Play mode. The builder now uses its supported `manualExit: true` mode, allowing the existing callback to capture the frame and terminate through `EditorApplication.Exit`. The resulting image is still required to pass the Metal visual contract. The project and all Unity CI lanes now use the serialized project version `6000.3.18f1` rather than mixing Unity 2022.3 and Unity 6.

The Android Unity classifier has the same completed-evidence rule: a secondary Firebase reporting error becomes a warning once every required image exists and passes, but it cannot mask a render, composition, stereo, external-screen, API-fallback, crash, or incomplete-evidence failure. Its Firebase and visual-metric steps are also non-terminal so the evidence classifier, rather than step ordering, determines the lane result.

Local classifier, report aggregation, workflow-matrix, composition-contract, and sample-entrypoint tests pass. The cleanup gate remains open until a fresh full cloud run is downloaded and every automatic verdict is reconciled against every image.
