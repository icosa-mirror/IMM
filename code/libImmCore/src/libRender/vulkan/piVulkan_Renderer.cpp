...
struct piVulkanState
{
    ...
    bool externalDepthClearConventionReported = false;
...
static bool iExternalReverseZActiveForTarget(piVulkanState *state, piRTarget target)
{
    // Reverse-Z is a Unity external-eye convention. Godot's compositor also
    // uses an external intermediate image, but supplies the standard
    // near-to-zero projection generated by ImmViewerNode. Treating that image
    // as reverse-Z makes far/back fragments win and exposes inside surfaces.
    return state && state->externalDepthReverseZ && !state->hostRenderPassFrameActive &&
...
bool piRendererVulkan::BeginExternalImageFrameWithView(void *image, void *imageView, uint32_t vkFormat, uint32_t colorVkSamples, void *depthImage, void *depthImageView, uint32_t depthVkFormat, uint32_t depthVkSamples, int width, int height, int arrayLayers, bool ownsColorImageView, bool ownsDepthImageView, bool clearColor, bool clearExternalDepth)
{
    ...
    // Match the clear to the depth compare that the player configured. Unity
    // uses Linear10/GREATER on Vulkan (including Windows paths without a
    // dedicated queue); Godot uses Linear01/LESS. The dedicated-eye override
    // retains the explicit Android Unity contract. Using the global Unity
    // feature flag alone cleared Godot to 0.0 while comparing LESS, rejecting
    // every IMM fragment before composition.
    const bool clearExternalDepthAsReverseZ =
        (mState->currentDepthState && !mState->currentDepthState->lessEqual) ||
        (mState->externalDepthReverseZ &&
         !mState->hostRenderPassFrameActive &&
         iExternalReverseZEnabled(mState));
    if (!mState->externalDepthClearConventionReported)
    {
        mState->externalDepthClearConventionReported = true;
        iReport(mReporter,
                clearExternalDepthAsReverseZ
                    ? "[IMM_EXTERNAL_DEPTH_CLEAR_20260804] convention=reverse-z clear=0"
