...
- `get_render_backend_diagnostics()` reports the configured and actual Godot rendering method/driver, RenderingDevice availability, selected IMM renderer API, whether the current runtime state is a macOS Metal adapter candidate, and whether the current runtime state is a Vulkan adapter candidate for the upcoming Windows/Android path.
