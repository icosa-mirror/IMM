...
run_case() {
    ...
    if [ "${IMM_METAL_VALIDATE_LIVE_EDIT:-0}" != "0" ]; then
        ...
        if ! grep -Eq "\[IMM_LIVE_EDIT_CREATE\].*revision=2 status=4 result=0 drawingCountBefore=.*drawingCountAfter=.*handleMatch=1" "$log_path"; then
            echo "$name did not present drawing creation with its reserved handle" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        ...
        if ! grep -Eq "\[IMM_LIVE_EDIT_KEY_SET\].*revision=5 status=4 result=0 keyCountChanged=1" "$log_path"; then
            echo "$name did not present the animation-key revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        ...
        if ! grep -Eq "\[IMM_LIVE_EDIT_INITIAL_SPAWN\].*revision=7 status=4 result=0 initialLayerMatch=1" "$log_path"; then
            echo "$name did not present the initial spawn-area revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        ...
        if ! grep -Eq "\[IMM_LIVE_EDIT_LAYER_CREATE\].*revision=9 status=4 result=0 layerMatch=1 countChanged=1" "$log_path"; then
            echo "$name did not present the group-layer creation revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        if ! grep -Eq "\[IMM_LIVE_EDIT_LAYER_CHILD_CREATE\].*revision=10 status=4 result=0 childMatch=1 countChanged=1" "$log_path"; then
            echo "$name did not present the child group-layer creation revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        if ! grep -Eq "\[IMM_LIVE_EDIT_LAYER_REPARENT\].*revision=11 status=4 result=0 orderMatch=1" "$log_path"; then
            echo "$name did not present the group-layer reorder revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        if ! grep -Eq "\[IMM_LIVE_EDIT_LAYER_CROSS_PARENT\].*revision=12 status=4 result=0 hierarchyMatch=1" "$log_path"; then
            echo "$name did not present the cross-parent group-layer revision" >&2
            tail -n 80 "$log_path" >&2
            exit 1
        fi
        if ! grep -Eq "\[IMM_LIVE_EDIT_LAYER_DESTROY\].*revision=13 status=4 result=0 subtreeMissing=1 countRestored=1" "$log_path"; then
