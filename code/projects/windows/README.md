...
The GitHub Actions Windows job additionally captures a DirectX standalone baseline, runs the Release Windows standalone Vulkan `sample1.imm` smoke against it, downloads Godot 4.5, caches `thirdparty\godot-cpp` by `GODOT_CPP_REF`, builds the GDExtension, runs `run-godot-smoke.ps1 -RequireExtension -PreflightOnly -LogDir artifacts\godot-extension-preflight`, then runs headed Windows Godot Vulkan playback smoke with intermediate-texture tracing enabled. CI uploads Windows Godot platform binaries as `ImmGodotGDExtension-Windows-platform` for the combined addon package, and uploads a separate `ImmViewerVulkan-Windows` standalone viewer artifact whose `settings.json` selects Vulkan and loads `sample1.imm`.
