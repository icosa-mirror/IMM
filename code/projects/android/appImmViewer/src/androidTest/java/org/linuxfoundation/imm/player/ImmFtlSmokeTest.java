...
public final class ImmFtlSmokeTest {
    ...
    public void rendersSampleFrame() throws Exception {
        ...
        requireMarker(logcat, "revision=11 status=4 result=0 hierarchyMatch=1");
        requireMarker(logcat, "revision=12 status=4 result=0 layerMissing=1 countRestored=1");
    ...
    private static String waitForNativeCapture(UiDevice device, File capture, int waitSeconds) throws Exception {
        ...
        while (SystemClock.elapsedRealtime() < deadline) {
            ...
            if (capture.isFile() && capture.length() > 0L &&
                    ...
                    (logcat.contains("revision=12 status=4 result=0 layerMissing=1 countRestored=1") ||
