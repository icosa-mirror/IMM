...
public final class ImmFtlSmokeTest {
    ...
    public void rendersSampleFrame() throws Exception {
        ...
        requireMarker(logcat, "revision=5 status=4 result=0 countRestored=1 deletedHandleMissing=1 originalHandleStable=1 referencedDeletionRejected=1");
        ...
        requireMarker(faceLogcat, "revision=5 status=4 result=0 countRestored=1 deletedHandleMissing=1 originalHandleStable=1 referencedDeletionRejected=1");
    ...
    private static String waitForNativeCapture(UiDevice device, File capture, int waitSeconds) throws Exception {
        ...
        while (SystemClock.elapsedRealtime() < deadline) {
            ...
            if (capture.isFile() && capture.length() > 0L &&
                    ...
                    logcat.contains("revision=5 status=4 result=0 countRestored=1 " +
