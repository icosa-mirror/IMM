param(
    [string]$Adb = $env:ADB,
    [int]$WaitSeconds = 30,
    [string]$LogDir = "logs/android-quest-vr-smoke",
    [string]$BuildDir = "build_vr",
    [switch]$SkipBuild,
...
function Require-Marker([string]$Log, [string]$Marker) {
    if (-not $Log.Contains($Marker)) {
        throw "Android Quest VR smoke log did not contain required marker: $Marker"
    }
}
...
function Test-BlockedLaunchState([string]$Log, [string]$ActivityText, [string]$PowerText) {
    if ($Log.Contains("Launch is blocked because: a Reprojected OS dialog is currently showing")) {
        throw "Android Quest VR smoke launch is blocked by a reprojected Quest OS dialog. Clear the visible headset dialog, then rerun. Log: $logPath"
    }
    ...
    if ($Log.Contains("RequiresControllersLaunchInterceptor") -or
        ...
        throw "Android Quest VR smoke launch is blocked by Quest controller-required launch check. Wake/connect controllers or use a hand-tracking-capable VR manifest, then rerun. Log: $logPath"
    }

    if ($ActivityText.Contains("com.oculus.os.vrlockscreen") -or
        $ActivityText.Contains("com.oculus.guardian")) {
        throw "Android Quest VR smoke launch is blocked by Quest OS focus state (VR lockscreen/Guardian). Wake/unlock the headset and clear Guardian/dialogs, then rerun. Log: $logPath"
    }

    if ($PowerText.Contains("mWakefulness=Asleep") -or $PowerText.Contains("mScreenState=OFF")) {
        throw "Android Quest VR smoke launch is blocked because the Quest display is asleep/off. Wake the headset and keep it active, then rerun. Log: $logPath"
    }
}
...
if (-not $SkipBuild) {
    ...
    try {
        & .\gradlew.bat :appImmViewer:assembleDebug -PimmNonVr=OFF "-PimmBuildDir=$BuildDir" -PimmManifest=vr
        if ($LASTEXITCODE -ne 0) {
            throw "Android Quest VR APK build failed with exit code $LASTEXITCODE"
        }
    }
    finally {
        Pop-Location
...
if (-not (Test-Path $apk)) {
    throw "Android Quest VR APK not found: $apk"
}

$devicesOutput = & $adbPath devices
$deviceLines = @($devicesOutput | Where-Object { $_ -match "`tdevice$" })
if ($deviceLines.Count -le 0) {
    throw "No Android/Quest device is attached for Quest VR runtime smoke."
}
...
if (-not $pidText) {
    throw "Android Quest VR process was not running after $WaitSeconds seconds. Log: $logPath"
}
...
$requiredMarkers = @(
    ...
    "vrapi_EnterVrMode()",
    "Checking for sample1.imm",
    "Loading Quill:",
    "Initialized Quill Viewer",
    "IMM_ANDROID_VR_SMOKE viewer_initialized",
    "Loaded in CPU",
    "Loaded in GPU",
    "loading completed... vrapi_SetClockLevels",
...
$forbiddenMarkers = @(
    "FAILED to open sample1.imm",
    "Asset directory is null or empty",
    "Can't init Quill Viewer",
    "Failed to load IMM",
    "Fatal signal",
    "F DEBUG"
)
...
foreach ($marker in $forbiddenMarkers) {
    if ($log.Contains($marker)) {
        throw "Android Quest VR smoke log contained failure marker: $marker"
    }
}

Write-Host "Android Quest VR smoke passed"
