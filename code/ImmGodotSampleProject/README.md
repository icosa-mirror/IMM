...
The Windows workflow runs a native-extension preflight after the GDExtension build. The preflight validates staged DLLs and Godot editor lookup paths; local Windows visual validation should also run `VisualSmokeScene.tscn` or press Play in the project to exercise the Vulkan compositor path.
