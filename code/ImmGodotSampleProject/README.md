...
The Windows workflow runs a native-extension preflight after the GDExtension build, then runs headed Windows Godot Vulkan playback smoke against `SampleScene.tscn` with intermediate-texture tracing enabled. The macOS workflow runs the Metal visual smoke harness and saves a PNG. Local visual validation can still run `VisualSmokeScene.tscn` directly, or press Play in the project to exercise the default platform compositor path.
