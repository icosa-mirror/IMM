...
- The Vulkan backend now has real device-local `VkImage` allocation and `VkImageView` creation for supported 2D renderer textures, plus swapchain image views, a color render pass, and swapchain framebuffers. That is the frame structure needed for replacing CPU render-target storage with Vulkan graphics pipelines.
