...
- Latest Windows Vulkan smoke verified player output through the real present path: after `sample1.imm` loaded and paint rasterization produced a nonblack target, `debug.txt` reported `Vulkan renderer presented swapchain nonblack texture frame`. This proves the visible paint output is no longer only the GDI bridge; it is also copied into an acquired swapchain image and presented with `vkQueuePresentKHR`.
