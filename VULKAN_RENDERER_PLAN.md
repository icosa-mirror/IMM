...
12. [in progress] Implement real Vulkan render targets, command buffers, render pass/framebuffer setup, and swapchain presentation for the Windows standalone viewer. The owned Windows path now creates a Win32 `VkSurfaceKHR`, selects a graphics queue that can present to it, enables `VK_KHR_swapchain`, creates a window-sized swapchain, enumerates swapchain images, creates frame command/sync resources, and submits/presents a cleared swapchain image in `SwapBuffers`; rendering the actual player color target into the swapchain is still pending.
