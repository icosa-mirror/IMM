...
- The Vulkan backend now has real device-local `VkImage` allocation for supported 2D renderer textures. That is a foundation step for replacing CPU render-target storage with Vulkan image views, framebuffers, render passes, and graphics pipelines.
...
- Current implementation progress: supported 2D texture formats now map to Vulkan formats and allocate device-local `VkImage` memory through dynamically loaded `vkCreateImage`, `vkGetImageMemoryRequirements`, and `vkBindImageMemory`.
