...
- Two-camera overlay diagnostics did not preserve the main camera color in the current Windows Vulkan editor smoke; the later camera output matched its own clear/overlay baseline even though the main camera native logs showed IMM draw calls. Treat the two-camera route as unresolved until the camera stacking/load-op behavior is understood.
...
- 2026-06-13 follow-up:
  ...
  - `artifacts/unity-vulkan-overlay-regression/candidate-vulkan-msaa8-default-second-camera-overlay.png` is the current inspected Unity Vulkan MSAA evidence for the ordered-overlay milestone. It uses the new defaults, a main camera filtered with `IMM_UNITY_CAMERA_NAME=Main Camera`, and a second Unity overlay camera. The image shows IMM picture/paint plus a red Unity cube composited in front. This proves the practical overlay-camera route, not full host-depth interleaving.
...
- Unity desktop Vulkan ordered overlay/background composition now has local evidence for the overlay-camera route with normal MSAA rendering: `artifacts/unity-vulkan-overlay-regression/candidate-vulkan-msaa8-default-second-camera-overlay.png`. Same-camera arbitrary Unity geometry over IMM is not yet a general feature; the validated route is main camera IMM plus a later Unity overlay camera or explicit command-buffer overlay draw.
