...
## 1.1 Status update (2026-09-19)

This plan was written on 2026-07-20 with Phases 0-6 recorded complete for Windows x64
paint authoring. The following has changed since, from the plugin/wrapper gap work
recorded in `docs/unity-plugins.md`:

- **Spawn areas are implemented**, not deferred: `ImmAuthoringLayerType.SpawnArea` with
  tracking level, sphere/box locomotion volume, per-axis translation mask and
  default-viewpoint selection is supported through the mutable graph, the compiler, the
  importer (via `StrokeReader_GetLayerSpawnAreaInfo`), the structural comparer and the
  native exporter (`ImmExporter_CreateSpawnAreaLayer`, `ImmExporter_SetInitialSpawnArea`,
  `ImmExporter_SpawnAreaSetProperties`). This closes Phase 7 increment 2.
- **Authoring is no longer Windows-only**: `libImmExporter` is built and linked on
  Android (arm64-v8a), iOS and macOS as well as Windows, `piWaveOPUS` is part of those
  core builds, and libopusenc 0.2.1 is vendored under `thirdparty/libopusenc-src` for the
  Opus audio path. `ImmAuthoringRuntime.Capabilities` now reports the authoring feature
  set on every platform (64-bit pointer width is the only requirement). This closes
  Phase 7 increment 8.
- **Two previously missing exporter entry points were implemented**:
  `ImmExporter_LayerAddAnimationKey` and `ImmExporter_PaintSetMaxRepeatCount` were called
  by `ImmAuthoringCompiler` but existed in no binary; both are now native, with the
  authoring export path verified end to end through the exporter smoke test.
- **The playback surface grew** beyond what §3.1 describes: chapter lengths and
  `hasPlays`, audio presence, load-time measurement, load cancellation, unload-all
  (sync and async), pause/resume at a tick, render counters, a full camera viewport
  override and a per-frame work budget.
- **A stroke reader sample scene** (`Assets/Scenes/StrokeReaderSampleScene.unity` plus
  `ImmStrokeReaderExample`) now exercises the whole reader surface from the sample
  project, which previously had no reader consumer at all.

Authoritative status now lives in two tracked documents; this plan is a local planning
record:

- `docs/unity-plugins.md` - per-platform plugin binaries, the full native API with C#
  wrapper status, and the gap fix log.
- `docs/runtime-authoring-engine-contract.md` - the engine contract. **It is still stale
  in the two dimensions above** (it states Windows x64 only, and lists spawn areas and
  cross-platform authoring as outside the contract); updating it is the next
  documentation task.

Known gaps at this date:

- The spawn-area authoring path has no Unity test coverage yet; the recorded PlayMode
  counts in Phases 5 and 6 predate it.
- Runtime authoring is still demonstrated only by the opt-in
  `Samples~/RuntimeAuthoring` package sample, not by a scene in `ImmUnitySampleProject`
  itself (`Assets/Samples` is gitignored).
- §11's logging prefixes are not implemented (see that section).

Work done since this status line was written:

- **The presentation-versus-content edit policy is now documented** in
  `docs/runtime-authoring-engine-contract.md` ("Presentation edits versus content
  edits"), with the live player API for each presentation edit and the rule that a
  preview replacement must have its layer overrides re-applied by the application.
- **The compile-and-reload pipeline is now measured per stage** (contract document,
  "Measured stage split"): graph construction is ~18 ms for the large corpus while
  serialization is 120 ms to memory and 510 ms to a file, the player's CPU load is
  240 ms, and the synchronous stroke-reader import is 431 ms. Reproducible with
  `code/appImmUnity/tests/exporter_benchmark.py`. Serialization and decode - not graph
  construction - are what an in-memory handoff or per-layer caching would remove.
...
The playback surface is wider than this section originally recorded. The plugin now also
exposes, through the same package:

- Chapter layout: per-chapter lengths in ticks and whether chapters come from real Play
  markers (`ImmDocument.TryGetChapterInfo`).
- Audio presence as distinct from volume (`ImmDocument.HasAudio`).
- Load control: cancellation of an in-flight load and the last load time in
  milliseconds (`ImmDocument.CancelLoading`, `ImmPlayerManager.LastLoadTimeMs`).
- Unload-all in one call, synchronous or not (`ImmPlayerManager.UnloadAllDocuments`).
- Frame-accurate playback control (`ImmDocument.PauseAt`/`ResumeAt`).
- Render counters: draw calls, culled draw calls, triangles and CPU/GPU timing, enabled
  explicitly because they cost work per frame (`ImmPlayerManager.GetPerformanceInfo`).
- Camera control: a full viewport override (origin, sub-rect size, depth range, force
  flag) and a per-frame native work budget (see `docs/unity-plugins.md`).
...
The current managed surface can create sequences, nested groups, paint layers, drawing,
stroke elements, stroke points, frame mappings and animation keys, set paint repeat
counts, create spawn-area (viewpoint) layers with their volume and default selection, and
export to an IMM file or an owned memory buffer.

The native exporter contains additional concepts that are still not exposed through the
Unity C ABI: model, picture and sound layer creation, effects, and references/instances.

The C ABI itself is no longer Windows-only. `libImmExporter` is linked by
`appImmUnity.vcxproj` on Windows and by the Android gradle/CMake module, the iOS archive
merge and the macOS bundle everywhere else; `piWaveOPUS` is part of the corresponding
core builds, with the libopusenc 0.2.1 sources vendored under
`thirdparty/libopusenc-src` (upstream ships autotools only, so the vendored target
compiles the same source set the Windows binaries use). The exporter block in
`appImmUnity/src/main.cpp` and its includes are unconditional, and all 97 entry points the
Windows plugin exports are compiled on the other platforms too.
...
It exposes hierarchy, transforms, drawings, strokes, points, frame buffers, animation metadata, and selected picture data, and since the spawn-area work it also exposes each spawn-area layer's tracking level, locomotion volume (sphere or box, offset and extents) and translation mask through `StrokeReader_GetLayerSpawnAreaInfo`. It is useful for paint import but is not currently a lossless general IMM round-trip API.
...
- 64-bit runtime authoring on Windows, macOS, Android, and iOS.
- New documents.
- Still and animated sequences.
- Group and paint layers.
- Spawn-area (viewpoint) layers: tracking level, sphere/box locomotion volume,
  per-axis translation mask, and default-viewpoint selection, through both the mutable
  graph and the exporter API.
...
Delivered since this plan was written (previously listed as deferred): spawn-area
viewpoints, and cross-platform authoring builds for macOS, Android, and iOS.
...
Status: completed on 2026-07-19 for the explicitly supported paint subset defined in
`docs/runtime-authoring-engine-contract.md`. The subset is no longer Windows-bound: the
same import path, including spawn-area viewpoints, is compiled for macOS, Android, and
iOS (2026-09), and only the contract document still describes it as Windows-only.
...
- Two final complete Windows Unity PlayMode runs passed 29 of 29 tests (as recorded on
  2026-07-19, before the spawn-area work; the current suite in `Tests/Runtime` declares
  33 `[Test]`, 6 `[UnityTest]` and 10 `[TestCase]` attributes, so these counts need
  re-recording on a fresh run). The extended
...
Status: completed on 2026-07-19 for the paint-animation engine, which since 2026-09 is
built for Windows, macOS, Android, and iOS rather than Windows x64 alone.
...
Recorded result (2026-07-19, before the spawn-area work): two final Windows Unity
PlayMode runs passed 37 of 37 tests
in 4.503 seconds and 4.152 seconds. The live sample reported all Phase 6 capability flags and
...
1. Complete animation-key support. *(Largely delivered: all ten properties exist and
   obsolete component position/rotation/scale keys are kept only for legacy import;
   what remains is validating unsupported property/interpolation combinations.)*
2. Spawn areas and viewpoints. *(Delivered 2026-09.)*
3. Pictures and 360-degree imagery. *(Open.)*
4. Sound and spatial-audio properties. *(Open.)*
5. Models. *(Open.)*
6. References and instances. *(Open.)*
7. Comic and chapter structures. *(Open; `ImmAuthoringCompiler` still returns
   `Unsupported` for `ExportSequenceType.Comic`.)*
8. macOS, Android, and iOS authoring builds. *(Delivered 2026-09.)*
...
Implementation status: only `[IMM_AUTHOR_PREVIEW]` exists today
(`ImmAuthoringPreviewCoordinator.cs`). The model, edit, compile, export and import
subsystems return structured `ImmAuthoringResult` failures instead of logging, which
matches the rule below; treat the remaining prefixes as the naming convention to use if
and when internal failures in those subsystems need logs.
...
Resolved in 2026-09: Windows authoring shipped first, the managed contract stayed
platform-neutral, capabilities are queried at runtime, and the additional native
platforms (macOS, Android, iOS) were added afterwards as one increment. The mitigations
below remain the rule for future platforms:
...
### Authoring validation is not layer-type aware for spawn-area fields

`ImmAuthoringDocument.ValidateLayerProperties` enforces the spawn-area volume rules
(positive extents) for every layer, even though only spawn-area layers use them.
Callers that build `ImmAuthoringLayerProperties` by hand instead of starting from
`ImmAuthoringLayerProperties.Default(...)` can therefore see a spawn-area error on a
paint or group layer. Scope the check to spawn-area layers when the spawn-area path gets
its test coverage.
...
| Production-ready paint-animation engine | 5–7 months cumulative |
| Broad IMM authoring engine and additional platforms | 8–14 engineer-months cumulative |

Status against these rows (2026-09-19): the first four are delivered, and the platform
half of the fifth (macOS, Android, iOS authoring builds) is delivered as well; what
remains of the fifth row is the broader IMM feature surface listed in Phase 7
(pictures, sound, models, references, instances, comic chapters).
...
Status: satisfied. Phases 0 and 1 completed on 2026-07-19, and Phases 2-6 followed; this
section is kept as the historical statement of the original first step.
