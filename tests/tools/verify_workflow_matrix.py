...
REQUIRED_JOBS = {
    ".github/workflows/build.yml": {
        ...
        "build-macos": ["Smoke macOS Metal standalone viewer", "Validate macOS Metal standalone audio", "Record macOS Metal render metrics", "Write macOS Metal render report", "Write macOS viewer artifact manifest", "Collect macOS viewer artifact summary"],
        "build-ios": ["Link-check iOS Unity plugin", "Verify iOS Unity audio pipeline contract"],
    ...
    ".github/workflows/ci-ios.yml": {
        "standalone-ios-metal": ["Prepare standalone iOS artifact directory", "Configure standalone iOS Simulator application", "Compile standalone iOS application without signing", "Boot deterministic iOS Simulator for standalone viewer", "Run standalone iOS Metal visual and lifecycle validation", "Compare standalone iOS Metal render evidence", "Write standalone iOS Metal render report", "Package standalone iOS arm64 release input after visual validation", "Write standalone iOS Metal visual manifest", "Collect standalone iOS Metal evidence", "Upload standalone iOS Metal evidence", "Enforce standalone iOS Metal result"],
    ...
    ".github/workflows/ci-engine.yml": {
        ...
        "unity-windows-vulkan-synthetic-stereo": ["Install Mesa lavapipe", "Configure Mesa lavapipe Vulkan ICD", "Preflight hosted Unity Vulkan stereo runner", "Run Unity Vulkan synthetic stereo smoke", "Classify Unity Vulkan synthetic stereo runtime", "Split and verify stereo visual evidence", "Verify Unity Vulkan synthetic stereo log contract", "Classify Unity Vulkan synthetic stereo evidence", "Stage Unity Vulkan synthetic stereo capture evidence", "Write CI manifest", "Collect artifact summary", "Upload Unity Vulkan synthetic stereo artifacts"],
    ...
    ".github/workflows/ci-gpu.yml": {
        ...
        "windows-godot-vulkan": ["Download Windows viewer build artifact", "Stage Windows viewer build artifact", "Download Windows Godot extension build artifact", "Stage Windows Godot extension build artifact", "Set up MSYS2 Vulkan tools", "Install pinned Mesa lavapipe", "Configure Mesa lavapipe Vulkan ICD", "Preflight Godot Vulkan runner", "Run Godot Vulkan render-only baseline smoke", "Run Godot Vulkan visual baseline smoke", "Record Godot Vulkan full depth metrics", "Run Godot Vulkan ordered overlay smoke", "Record Godot Vulkan ordered overlay metrics", "Classify Windows Godot Vulkan evidence", "Write Godot Vulkan render report", "Write Godot Vulkan render baseline report", "Write Godot Vulkan ordered overlay report", "Write Godot Vulkan ordered overlay baseline report", "Write CI manifest", "Collect artifact summary"],
        ...
        "macos-godot-metal": ["Preflight Godot Metal runner", "Download macOS Godot extension build artifact", "Stage macOS Godot extension build artifact", "Run Godot Metal visual smoke", "Verify Godot Metal audio pipeline", "Record Godot Metal render metrics", "Classify Godot Metal visual evidence", "Write Godot Metal render report", "Write Godot Metal composition report", "Write CI manifest", "Collect artifact summary"],
...
REQUIRED_RUNS_ON = {
    ...
    ".github/workflows/ci-engine.yml": {
        ...
        "unity-windows-vulkan-synthetic-stereo": {"windows-latest"},
...
REQUIRED_JOB_TIMEOUTS = {
    ...
    ".github/workflows/ci-engine.yml": {
        ...
        "unity-windows-vulkan-synthetic-stereo",
...
REQUIRED_STEP_TIMEOUTS = {
    ".github/workflows/ci-ios.yml": {
        "godot-ios-package": {"Run ordinary Godot iOS sample on Firebase Test Lab"},
    },
    ".github/workflows/ci-device.yml": {
...
REQUIRED_ALWAYS_STEPS = {
    ...
    ".github/workflows/ci-engine.yml": {
        ...
        "unity-windows-directx-composition": {
            ...
            "Classify Unity DirectX visual evidence",
            "Verify Unity DirectX composition log contract",
        },
        "unity-windows-vulkan-synthetic-stereo": {
            "Classify Unity Vulkan synthetic stereo runtime",
            "Split and verify stereo visual evidence",
            "Verify Unity Vulkan synthetic stereo log contract",
            "Classify Unity Vulkan synthetic stereo evidence",
        },
        "unity-windows-vulkan-ordered-overlay": {
            "Record Unity Vulkan ordered overlay metrics",
            "Classify Unity Vulkan ordered overlay visual evidence",
            "Write Unity Vulkan ordered overlay report",
        },
        "unity-windows-vulkan-full-depth": {
            "Record Unity Vulkan full depth metrics",
            "Classify Unity Vulkan full depth visual evidence",
            "Write Unity Vulkan full depth report",
        },
    },
    ".github/workflows/ci-device.yml": {
        "android-standalone-gles": {
    ...
    ".github/workflows/ci-gpu.yml": {
        ...
        "macos-godot-metal": {
            ...
            "Classify Godot Metal visual evidence",
        },
    },
}
...
def verify_strict_visual_validation_contract(path: Path, workflow_rel: str, errors: list[str]) -> None:
    required_by_workflow = {
        ...
        ".github/workflows/ci-engine.yml": [
            ...
            "classify_unity_synthetic_stereo.py",
            "--classification-json artifacts\\unity-windows-directx-composition\\composition-status.json",
            "--classification-json artifacts\\unity-windows-vulkan-ordered-overlay\\composition-status.json",
