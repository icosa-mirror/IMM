...
- **Completed so far:**
  ...
  - Wired the Windows GitHub Actions job to download Godot 4.5, run the script-stub smoke before building the GDExtension, and run the native smoke with two repeated load/unload cycles after building the GDExtension.
  ...
  - Fixed shared IMM backend initialization for headless/no-audio environments by falling back to the null sound backend when the platform sound backend is unavailable or fails to initialize. This matches the standalone viewer's validation behavior and prevents Windows CI runners without audio devices from blocking non-audio Godot load/render smoke coverage.
...
- **Remaining for Phase 1 completion:**
  - Commit and push the local no-audio fallback fix, then rerun the Windows CI GDExtension build and native Godot smoke with `godot-cpp` bootstrap enabled. The last remote run (`26422828235`, commit `935f214`) proved the extension-load path fix but failed because the Windows runner reported no audio devices and the shared backend treated that as fatal.
...
- **Visual Metal smoke status:** `MetalVisualSmokeScene.tscn` now runs under Godot Forward+/Metal on macOS, attaches `ImmViewerCompositorEffect` to the active camera, loads `sample1.imm`, performs one reload cycle by default, saves a 1600x900 PNG, and passes locally on Godot 4.6.1. The latest local run after the compositor orientation and timeline-safety smoke updates reported 1280 content pixels, a 52x43 content bound, luma range 1.0, `sequence_ready=true`, `last_metal_frame_started=true`, `last_had_intermediate_texture=true`, and `last_composite_result=true`. The compositor now samples the Godot-owned IMM Metal intermediate texture with RD's native texture orientation, without adding an extra vertical flip during the final scene-color composite.
