...
- **Visual Metal smoke status:** `MetalVisualSmokeScene.tscn` now runs under Godot Forward+/Metal on macOS, attaches `ImmViewerCompositorEffect` to the active camera, loads `sample1.imm`, performs one reload cycle by default, saves a 1600x900 PNG, and passes locally on Godot 4.6.1. The latest local run reported 1286 content pixels, a 52x43 content bound, luma range 1.0, `sequence_ready=true`, `last_metal_frame_started=true`, `last_had_intermediate_texture=true`, and `last_composite_result=true`. The compositor shader samples the intermediate Metal texture without a vertical V flip so the final Godot scene image is not upside down.
