...
- **Completed so far:**
  ...
  - Added `verify_local.py` and wired `build-godot-extension.ps1 -VerifyOnly` through it so local Godot extension API, `.gdextension` manifest, Forward+ project default, Run-button Metal visual main scene, native `ImmViewerNode` registration, `ImmViewerNode` method binding coverage, sample/native scene structure, `ImmViewerNode` camera registration plus camera/viewport render queue ownership, `ImmGodot` C ABI export alignment, Python, and native syntax checks can run before MSBuild/SCons/`godot-cpp` are installed.
